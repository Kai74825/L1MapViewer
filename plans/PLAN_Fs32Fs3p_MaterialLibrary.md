# fs32 / fs3p 格式與資材庫系統規劃

## 概述

實作兩種新格式用於地圖資料轉移：
- **fs32** - 地圖打包格式（S32 + Tiles）
- **fs3p** - 資材庫格式（可選 Layer + Tiles，跨地圖共享）
...
| `MaterialLibraryPath` | `Documents\L1MapViewer\Materials` | 資材庫存放路徑 |
...
| `MaterialLibrary.cs` | 資材庫管理（索引、搜尋、最近使用） |
...
| `MaterialBrowserForm.cs` | 完整資材瀏覽器視窗 |

> 注意：資材面板直接在 `MapForm.Designer.cs` 中新增控件（lblMaterials, lvMaterials, btnMoreMaterials），不需要獨立的 UserControl。
...
| `MapForm.cs` | 刪除 Layer4 群組篩選相關方法、新增資材面板事件處理、右鍵選單項目、資材貼上預覽 |
...
│   ├── txtTileSearch
│   └── lvTiles
├── Layer4 群組篩選 (y: 185-330) ← 刪除，替換成資材面板
│   ├── lblLayer4Groups        ← 刪除
│   └── lvLayer4Groups         ← 刪除
└── 群組縮圖列表 (y: 335-645)
    ├── lblGroupThumbnails
...
| 14 | `Helper/MaterialLibrary.cs` | 資材庫索引管理 |
| 15 | `MapForm.cs` | 資材面板事件處理、右鍵選單整合 |
| 16 | `Forms/ExportOptionsDialog.cs` | 匯出選項對話框 |
| 17 | `Forms/MaterialBrowserForm.cs` | 完整資材瀏覽器 |
| 18 | `Models/EditState.cs` | 資材預覽狀態 |
